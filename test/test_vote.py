...
def test_delete_vote_on_non_existing_post(authorized_client, test_posts):
    res = authorized_client.post("/vote/", json={"post_id": test_posts[3].id, "dir": 0})
    assert res.status_code == 404

def test_vote_on_non_existing_post(authorized_client, test_posts):
    res = authorized_client.post("/vote/", json={"post_id": 9999999, "dir": 0})
    assert res.status_code == 404

def test_unauthorised_user_vote_on__post(client, test_posts):
